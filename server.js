...
function rnd(n = 32) {
  return b64(crypto.randomBytes(n));
}
...
app.get("/auth/deriv", async (_req, res) => {
  try {
    reqEnv("DERIV_CLIENT_ID", CLIENT_ID);
    reqEnv("DERIV_CLIENT_SECRET", CLIENT_SECRET);
    reqEnv("DERIV_REDIRECT_URI", REDIRECT_URI);
    reqEnv("SESSION_SECRET", SESSION_SECRET);
